...
const { age, name } = student;  // name과 age라는 key 이름으로 구조분해
console.log(name, age);

// name의 키 값을 learnerName이라는 새 변수에 담았다는 뜻
const { name:learnerName,job = '학생'} = student; //원본에 job을 추가하는것은 아님
console.log(learnerName);
console.log(job);   //student 객체에 존재하지 않는 값이기 때문에 undefined;  
...
//  const {name:name3, price=price3} = product;
...
const productNames1 = products.map(  ({name}) => name); // 첫():화살표함수 두번째():매개변수 {}:객체구조분해
